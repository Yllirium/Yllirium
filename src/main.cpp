...
#if defined(NDEBUG)
# error "Yllirium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0dfb4bb44a634f05bd29aaa412d7fe8b84b69f6ec25e1a07e5f40c40eb60d7bb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yllirium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yllirium Signed Message:\n";
...
{
    // Yllirium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 480000 blocks
    nSubsidy >>= (nHeight / 480000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yllirium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yllirium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1520349637, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "SPIEGEL ONLINE 03.03.2018 Jupp Heynckes Der Held der aus der Rente kam";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 75 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049dd0323365f41e8c900424764b8c76f0528ed96daaffac6a038bc62980d8ebca2ec0c17d2af830b25ca93904f4949dc67b2a061cc785a226696ee6edd83e4d8b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520349637;
        ...
        assert(block.hashMerkleRoot == uint256("0x3d09edde0344607276588ff37caa824b47cae53a8c3f9b6bba6da8d45bbd22b0"));
...
unsigned char pchMessageStart[4] = { 0xbb, 0x9f, 0x82, 0x70 }; // Yllirium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YlliriumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YlliriumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YlliriumMiner(CWallet *pwallet)
{
    printf("YlliriumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yllirium-miner");
    ...
    try { loop {
        ...
        printf("Running YlliriumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YlliriumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YlliriumMiner, pwallet));
}
